...
func (db *DB) Find(out interface{}, where ...interface{}) *DB {
  ...
  case reflect.Slice:
    if v, ok := reflect.TypeOf(out).Elem().(transaction.Transaction); ok {
      ...
      append(s, transaction.Transaction{ID: 1, Timestamp: t1, Amount: 10,}, transaction.Transaction{ID: 1, Timestamp: t2, Amount: -5,})
    } else {
      db.logger.Println("Mockdb Find does not handle the provide slice.")
    }
  default:
