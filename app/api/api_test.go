...
func TestEndpointsBadDB(t *testing.T) {
  t1, err := time.Parse(time.RFC3339, "2019-01-30T03:17:41.12004Z")
  if err != nil {
    panic(err) 
  }
  ...
  tests := []struct {
  ...
  }{
    {
      ...
      expectedBody: string(generateJsonByteArray(problem.Problem{Status: 500, Title: "Internal Server Error", Detail: "Cannot get transactions; database not initialized.", Type: "about:blank",})[:]),
    },
    {
      name: "transactions_post",
      in: httptest.NewRequest("POST", "/transactions", nil),
